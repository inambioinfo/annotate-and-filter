...
- Use `ggplot2` to visualise our filtering results
...
`annovar` also has a series of filter that can help us to [predict the consequence](http://annovar.openbioinformatics.org/en/latest/user-guide/filter/#ljb42-dbnsfp-non-synonymous-variants-annotation) of a particular mutation. 
...
```{r}
anno <- mutate(anno, Chr =paste0("Chr", Chr))
anno
```
...
Multiple conditions can be separated by a `,`. This is the same as using `&` in base R.
...
A slighty annoyance with theexercise above is that once we've filtered for COSMIC variants we don't actually see the COSMIC column appearing in the output:-
...
- Print the chromsome, start, end, Gene name and 1000 genomes frequency and GWAS catalogue details for these variants
...
We might also want to incorporate quality information from the vcf file into the results. In many real life situations, data are spread across multiple tables or spreadsheets. Usually this occurs because different types of information about a subject are collected from different sources. It may be desirable for some analyses to combine data from two or more tables into a single data frame based on a common column, for example, an attribute that uniquely identifies the subject.
...
![](images/dplyr_left_join.png)

To illustrate join operations we'll consider the most common type, a "left join". In the schematic below the two data frames share a common column, V1. We can combine the two data frames into a single data frame by matching rows in the first data frame with those in the second data frame that share the same value of V1.
...
To put this into practice, we are going to join the annotation data frame we have just created with extra information available in the `.vcf` file

We can first import the `.vcf` file as we saw yesterday.
...
We can create a new data frame comprised of the ranges information for the variants and the `INFO` matrix. Using the the new `tbl_df` function

```{r}
extra.info <- tbl_df(data.frame(rowRanges(vcf),info(vcf)))
extra.info
```

In order to join the two tables we need to have variables that are in common. In our case the combination of chromosome, start and end position should be unique. We have to rename these columns so they are the same as the equivalent columns in `anno`

```{r}
extra.info <- mutate(extra.info, Chr=seqnames) %>% 
  mutate(Start=start) %>% 
  mutate(End=end)
extra.info 
```

We can now complete the join. The function should automatically detect which columns are in-common. If we want to be more explicit we can use the `by` argument.

```{r}
anno.extra <- left_join(anno, extra.info)
