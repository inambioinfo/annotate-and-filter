...
The `.vcf` file we are going to be using is `HCC1143_vs_HCC1143_BL.flagged.muts.vcf`, which we can quickly inspect using `VariantAnnotation`
...
Variants found in breast cancer according to COSMIC

```{r}
filter(anno, grepl("breast",cosmic76))
```
...
filter(anno, Func.refGene=="exonic") %>% 
  ...
  filter(X1000g2015aug_all < 0.05 | is.na(X1000g2015aug_all)) %>% 
  select(Chr:ExonicFunc.refGene,-GeneDetail.refGene,-Func.refGene,X1000g2015aug_all) %>% 
  arrange(desc(X1000g2015aug_all))
```

If we wish we can write the table we have just created to a `.csv` file
...
filter(anno, Func.refGene=="exonic") %>% 
  ...
  select(Chr:ExonicFunc.refGene,-GeneDetail.refGene,-Func.refGene,X1000g2015aug_all) %>% 
  arrange(desc(X1000g2015aug_all)) %>% 
  write.csv("filtered-variants.csv")
```
